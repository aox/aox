...
/*! Parses \a dt as an RFC 3339 (ISO 8601) format date and sets it. */
...
//  date-time       = full-date "T" full-time
    ...
    d->second = p.number();
    if ( p.present( "." ) ) {
        // we drop the fractional blah, we can't represent it
        // anyone. and we don't even care about rounding it off in the
        // right direction.
        (void)p.digits( 1, 25 );
    }
    p.require( "Z" );
    ...
    uint timeOffsetHour = p.number();
    p.require( ":" );
    uint timeOffsetMinute = p.number();
    d->tz = 60 * timeOffsetHour + timeOffsetMinute;
    if ( negative )
        d->tz = -d->tz;
